...
매주 **월·수·금 오전 9시(KST)** 에 포스팅이 실행되며, 요일마다 글의 출처와 "새로운 글을 가져오는 방식"이 다릅니다. (관련 로직: [`bot.js`](bot.js), 스케줄: [`.github/workflows/auto-post.yml`](.github/workflows/auto-post.yml))

| 요일 | 카테고리 | 내용 | 출처 | 새 글을 가져오는 방식 | 중복 방지 |
|------|----------|------|------|----------------------|-----------|
| **월** | 긍정 문구 | 오늘의 명언 | 외부 API (`korean-advice-open-api`) | API 호출 시마다 **무작위(random)** 로 명언 1개 반환 | ⚠️ **보장 안 됨** — 이전 게시 이력을 기록하지 않아 이론상 같은 명언이 다시 나올 수 있음 |
| **수** | 긍정 문구 | 알면 도움이 되는 생활 정보 | 로컬 파일 (`data/life-tips.json`) | 연중 **주차(week-of-year) 번호 % 목록 개수** 로 다음 항목을 순서대로 선택 | ✅ **목록 한 바퀴까지 중복 없음** (현재 12개 → 약 12주 주기로 순환) |
| **금** | 동료 칭찬 | ***을 칭찬합니다. | 로컬 파일 (`data/coworkers.txt`) | 목록에서 **무작위(random)** 로 동료 1명 선택 | ⚠️ **보장 안 됨** — 무작위 선택이라 직전과 같은 사람이 뽑힐 수 있음 |

> [!NOTE]
> - **수요일**만 "주차 기반 순환"이라 목록을 다 쓰기 전까지 중복이 없습니다. 새 생활정보를 추가하려면 `data/life-tips.json`에 `{ "title": "...", "content": "..." }` 항목을 더 넣으면 순환 주기가 길어집니다.
> - **월요일·금요일**은 매번 무작위라 "지난주와 다른 글"이 **확정적으로 보장되지는 않습니다.** 완전한 중복 제거가 필요하면 게시 이력을 파일에 기록해 재시도하는 방식으로 개선할 수 있습니다.
...
3. **Secrets** 탭(기본값)인지 확인합니다. (Variables 탭이 아님을 주의하세요!)
